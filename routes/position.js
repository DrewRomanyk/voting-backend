...
const requiredProperties = ["current_position", "confidence", "source_url", "source_quote", "source_date", "submit_status"];
const requiredCreateProperties = ["candidate_id", "issue_id", "submit_user_id", "submit_timezone"].concat(requiredProperties);
...
router.patch('/:candidate_id/:issue_id', (req, res) => {
    ...
    db.one('UPDATE voterapp.position SET current_position = ${current_position}, confidence = ${confidence}, source_url = ${source_url}, source_quote = ${source_quote}, source_date = ${source_date}, submit_status = ${submit_status} WHERE candidate_id = ${candidate_id} AND issue_id = ${issue_id} RETURNING *', {
